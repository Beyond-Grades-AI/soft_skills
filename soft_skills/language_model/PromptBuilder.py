...
def create_empathy_eval_prompt(question_text, answer, empathy_points):
    empathy_evl_prompt = f"עבור השאלה הבאה:\n" \
                         ...
                         f"{answer}\n" \
                         f"תנתח את התשובה לפי הנקודות הבאות לתשובה אחת, בנוסף תתחיל את התשובה בכן אם הטקסט מראה על אמפתיה ולמה, או תתחיל את התשובה בלא אם אין אמפתיה בטקסט ולמה, אם אין מגבלת טקסט אל תוסיף את זה לתשובה את זה לתשובה, אם בתשובה הנתונה אין שאלה המעמיקה את ההבנה אל תוסיף את זה לתשובה: \n" \
                         f"{empathy_points}"
